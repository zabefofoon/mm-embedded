<template>
  <div
    class="tw-w-full tw-h-screen | tw-absolute tw-top-0 tw-z-1 | tw-flex tw-flex-col | tw-bg-white | tw-transition-all"
    :class="widgetStore.isLayerShow ? 'tw-left-0' : 'tw-left-full'">
    <HeaderGoWidgetsButton class="tw-absolute tw-top-1 tw-right-0" />
    <HeaderGoWidgetsButton
      class="tw-absolute tw-top-1 tw-left-0 tw--translate-x-full" />
    <iframe
      class="tw-w-full tw-h-full"
      src="https://zabefofoon.github.io/dsm-embbedded/?edit=true"
      allow="clipboard-read; clipboard-write"
      @load="iframeLoadHandler($event)"></iframe>
  </div>
</template>
...
import {
  postProjectSaveProject,
  postProjectWidgetGroupsMutation
} from '../messenger/postToProject.msg'
...
onBeforeUnmount(() =>
  window.removeEventListener('message', listenWidgetLayerMessage)
)
...
const iframeLoadHandler = (event: Event) => {
  if (event.target) widgetStore.setWidgetEditor(<HTMLIFrameElement>event.target)
