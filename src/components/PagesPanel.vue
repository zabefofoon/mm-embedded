  ...
  <aside class="tw-w-80 | tw-border-r | tw-p-3">
    ...
    <ul class="tw-flex tw-flex-col tw-gap-2">
      <li
        ...
        <button
          class="w-full | tw-flex tw-items-center tw-gap-1 | tw-text-sm tw-text-left tw-text-gray-400"
          :class="[
            { 'tw-text-slate-500': page.id === pageStore.currentPageId },
            { 'tw-font-bold': page.id === pageStore.currentPageId }
          ]"
          @click="pageStore.selectPage(page.id)">
          <i class="mm-icon mm-icon-page"></i>
          <input
            class="tw-px-1 | tw-w-full"
            v-model="page.name" />
        </button>
      </li>
    </ul>
